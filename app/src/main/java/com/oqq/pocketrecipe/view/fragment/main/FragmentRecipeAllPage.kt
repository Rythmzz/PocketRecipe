...
class FragmentRecipeAllPage: Fragment() {
        ...
        firstThread = lifecycleScope.launchWhenStarted {
            this.launch {
                recipeViewModel.success.collect{
                    ...
                    binding.countRecipeMain.text = "Hơn +${recipeViewModel.countMainRecipe} ý tưởng công thức"
                    binding.countRecipeSub.text = "Hơn +${recipeViewModel.countSubRecipe} ý tưởng công thức"
                    binding.countRecipeOther.text = "Hơn +${recipeViewModel.countOther} ý tưởng công thức"
                }
                }
            }
        }
    ...
    private fun setBehavior() {
        ...
        for (i in 0 until listLayout.size){
            listLayout[i].setOnClickListener {
                val action =
                    FragmentRecipeAllPageDirections.actionFragmentRecipeAllDestToFragmentRecipeAllDetailDest(
                        i
                    )
    ...
    private fun setIntialData() {
        ...
        binding.textQuote.text = quotes[randomQuote]
