...
class SliderRecipeAdapter(
    private var listMeal: MutableList<Meal>
): SliderViewAdapter<SliderRecipeAdapter.SliderAdapterAddImage>() {
    ...
    inner class SliderAdapterAddImage(private val binding: ImageRecipeItemBinding): ViewHolder(binding.root){
        @SuppressLint("SetTextI18n")
        fun bind(item:Meal){
            binding.imageRecipe.setImageResource(item.image)
            binding.title.text = "Công thức món ${item.name} ngẫu nhiên"
